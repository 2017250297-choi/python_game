...
class WindowManager:
    ...
    def battle_window(self,player_entity:Player,enemy:Monster):
        ...
        while True:
            ...
            print(f'HP\t{player_entity.hp}/{player_entity.max_hp} \t\t {enemy.hp}/{enemy.max_hp}')
...
def make_monster():
    ...
    row=csv_data[dice]
    monster_entity=Monster(row['class_name'],'',int(row['hp']),int(row['power']),int(row['speed']),int(row['mag_d']))
    csv_file.close()
